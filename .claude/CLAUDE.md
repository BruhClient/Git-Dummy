...
- **For lane-assignment bugs, test slot-allocation fixes before attribution fixes** — changes to lane slot preference (`for p in parents[1:]:`, `lane_idx is None`) are self-contained and don't affect other branches; `commit_owner` attribution changes interact via first-write-wins and can silently steal commits from other branches if the BFS processes entries in the wrong order.
- **Before implementing any `commit_owner` attribution fix, print which commits it would claim and their current owner** — if any targeted commit already has a different named owner, the fix will cause a first-write-wins regression; one diagnostic print before implementing reveals this.
- **For wrong-button-visibility bugs, fix the flag computation in `commit_view.py`, not the gate in `detail_panel.py`** — `set_commit_actions` inputs (`is_first_of_branch`, `is_head`, etc.) are authoritative data computed in `commit_view.py`; patching the `if is_first_of_branch:` block in `detail_panel.py` just shifts the symptom without fixing the source. (e.g. removing the gate caused all three buttons to show simultaneously instead of fixing the wrong `is_first` value.)
- **Before using `branch` (from `_local_tip_branch`) in any count or comparison, verify it equals `commit.branch`** — `_local_tip_branch.get(sha, commit.branch)` can return an unexpected local branch name when multiple local branches share a tip SHA (whichever `for-each-ref` lists first wins). `commit.branch` is the lane algorithm's stable canonical name and should be preferred for membership counts. (e.g. `branch='sdfsdf'` instead of `'zen'` made `branch_commit_count=0`, keeping `is_first=True`.)
- **For async-check-during-navigation features, ask "which page is the user on while the check runs?" before implementing** — getting this wrong causes a full architectural revert (signals, emit sites, slot handlers). (e.g. implemented deferred navigation when user wanted CommitViewPage to show immediately with the check running on the loading screen.)
- **For "X visible through covering overlay" bugs, grep `.hide()` callers on the overlay before checking geometry or z-order** — the overlay may be getting explicitly hidden by an async load completion that bypasses the guard, not by layout issues. (e.g. FS watcher fired `_reload_debounce` → `_start_load()` → `_on_loaded()` → `_loading.hide()`, exposing the canvas before the dialog appeared.)
...
- **Using a merge commit's own owner as `attr` for its 2nd-parent chain** — sync-merge commits ("Merge branch 'main' into X") have 2nd parents from main's history whose neighboring commits may legitimately belong to other branches; BFS fallback 4 (`commit_owner[mc_sha]` as attr) caused detection to steal Shaun and univariate-only commits before the BFS could correctly attribute them. Never use `commit_owner[mc_sha]` as a BFS attr fallback.
